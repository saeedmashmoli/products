<template>
    <div class="container">
        ...
        <div v-if="!isLoading" class="main">
            <div v-for="(video,index) in videos" class="card" style="width: 18rem;">
                <img class="card-img-top" :src="video.picUrl" alt="Card image cap">
                <div class="card-body">
                    <h6 class="card-title">{{video.title}}</h6>
                    <p class="card-text">{{video.text.slice(0,100)}} ...</p>
                    <router-link :to="`/video/${video.slug}`" class="mt-2 mb-2">مشاهده</router-link>
                    <div class="float-right w-100">
                        <p class="mt-2 float-right"><small class="text-muted">{{video.publish_date}}</small></p>
        ...
        <div v-if="videoLastPage > 1" class="divider"></div>
...
<script>
    ...
    export default {
        ...
        methods : {
            ...
            getData(){
                this.isLoading = true;
                axios.post('/getAllVideos' , { 'page' :this.selectPage ,'categories' : this.$store.state.selectCategories})
                    .then((response) => {
