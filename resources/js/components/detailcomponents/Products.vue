<template>
    <div class="container">
        ...
        <div v-if="!isLoading" class="main">
            <div v-for="(product,index) in products" class="card" style="width: 18rem;">
                <img class="card-img-top" :src="product.url" alt="Card image cap">
                <div class="card-body">
                    <h6 class="card-title">{{product.title}}</h6>
                    <p class="card-text">{{product.text.slice(0,70)}} ...</p>
                    <router-link :to="`/product/${product.slug}`" class="mt-2 mb-2">مشاهده</router-link>
                    <div class="float-right w-100">
                        <p class="mt-2 float-right"><small class="text-muted">{{product.created_at}}</small></p>
        ...
        <div v-if="productLastPage > 1" class="divider"></div>
...
<script>
    ...
    export default {
        ...
        computed : {
            ...mapState(['products' , 'productLastPage']),
        },
        methods : {
            pageChangeHandler() {
                this.getData()
            ...
            getData(){
                this.isLoading = true;
                axios.post('/getAllProducts' , { 'page' : this.selectPage ,'categories' : this.$store.state.selectCategories})
                    .then((response) => {
